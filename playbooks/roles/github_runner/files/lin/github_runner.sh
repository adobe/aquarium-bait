...
[ "$CONFIG_FILE" ] || CONFIG_FILE=/mnt/ws/config/github_runner.env
...
if [ ! -d "$HOME/github_runner" ]; then
    ...
    while true; do
        ...
        runner_url=$(curl -s ${curl_insecure} "$list_url" | fgrep '"browser_download_url":' | grep -o 'https://.*actions-runner-linux-[^"]*' | fgrep -- "-${down_arch}-" | head -1 || true)
